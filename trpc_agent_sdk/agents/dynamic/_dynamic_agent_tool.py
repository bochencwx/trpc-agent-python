...
#
"""DynamicAgentTool — on-the-fly sub-agent creation where the LLM defines the role."""
...
_DESCRIPTION = (
    "Run one short-lived sub-agent for a single focused task and return "
...
class DynamicAgentTool(BaseTool):
    ...
    def __init__(
        ...
        super().__init__(name=name, description=description or _DESCRIPTION,
    ...
    def _get_declaration(self) -> FunctionDeclaration:
        properties: dict = {
            ...
            "instruction": Schema(
                type=Type.STRING,
                description=(
                    "Optional role, constraints, and execution guidance "
    ...
    async def process_request(
        ...
        *,
        ...
        include_parent_history = (
            self._agent_config is not None and self._agent_config.include_parent_history
        )
        if include_parent_history:
            instruction = (
                f"When using `{self.name}`: The sub-agent can see the "
            ...
            instruction = (
                f"When using `{self.name}`: The sub-agent has no memory "
