...
def load_archetype_from_file(path: Path,
                            tool_mapping: dict[str, Any] | None = None) -> SubAgentArchetype:
    ...
    if not instruction:
        raise ValueError(
            f"{path}: instruction body (text after the closing '---') must be non-empty"
        )
    ...
    if raw_tools is None:
        ...
        if not isinstance(raw_tools, list):
            raise ValueError(
                f"{path}: frontmatter 'tools' must be a YAML list, got {type(raw_tools).__name__}"
            )
...
def load_archetypes_from_dir(directory: os.PathLike,
    ...
    if errors:
        joined = "\n  ".join(errors)
        raise ValueError(
            f"Failed to load archetypes from {dirpath}:\n  {joined}"
        )
