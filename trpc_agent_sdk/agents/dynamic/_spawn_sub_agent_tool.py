...
class SpawnSubAgentTool(BaseTool):
    ...
    def __init__(
        ...
        for archetype in agents or []:
            if archetype.name in registry:
                raise ValueError(
                    f"archetype name {archetype.name!r} collides with an "
        ...
        if agent_paths is not None:
            for path in agent_paths:
                for archetype in load_archetypes_from_dir(path, tool_mapping=tool_mapping):
                    if archetype.name in registry:
                        raise ValueError(
                            f"archetype name {archetype.name!r} from {path!r} "
            ...
            parameters=Schema(
                ...
                properties={
                    ...
                    "description": Schema(
                        type=Type.STRING,
                        description=(
                            "Short label (3-7 words) of what this sub-agent will do."
                        ),
                    ),
                },
    ...
    async def _run_async_impl(
        ...
        *,
        ...
        archetype = self._registry.get(resolved_type)
