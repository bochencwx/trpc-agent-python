...
#
"""Frozen archetype template describing one kind of sub-agent the parent may spawn.

The archetype locks down instruction / tools / model so a prompt-injected call
cannot reshape the sub-agent into something arbitrary; only ``prompt`` varies
at call time.
"""
...
class SubAgentArchetype:
    ...
    - ``description`` is read by the **parent LLM** when it decides which
      archetype to spawn. It is rendered into the ``dynamic_agent`` tool
      description with a trailing ``(Tools: ...)`` suffix appended by the
      framework. Phrase it third-person, focused on selection criteria:
    ...
    - ``tools``: ``None`` = inherit all parent-agent tools at spawn time
      (minus ``DynamicAgentTool``, which is always stripped).
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.name, str) or not _NAME_RE.match(self.name):
            raise ValueError(
                f"SubAgentArchetype.name must match {_NAME_RE.pattern!r}, got {self.name!r}"
            )
