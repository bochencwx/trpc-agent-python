...
#
"""Orchestrator configurations demonstrating DynamicAgentTool.

Two configurations are provided, selectable via ``--mode`` on the command line:

- ``minimal`` — workspace tools and ``dynamic_agent`` are both registered on
  the orchestrator; the sub-agent inherits the parent surface and the model
  narrows tools per call.
- ``bounded`` — only ``dynamic_agent`` is registered; workspace tools live
  behind ``DynamicAgentTool(tools=...)`` as the capability surface.
"""
...
def create_minimal_agent() -> LlmAgent:
    """Orchestrator with workspace tools + dynamic_agent (Go ``-mode=minimal``)."""
    ...
    return LlmAgent(
        ...
        tools=workspace_tools + [DynamicAgentTool(skip_summarization=True)],
    )


def create_bounded_agent() -> LlmAgent:
    """Orchestrator with only dynamic_agent (Go ``-mode=bounded``)."""
    workspace_tools = create_workspace_tools()
    return LlmAgent(
        name="orchestrator",
        description="Orchestrator that delegates every subtask via dynamic_agent.",
        model=_create_model(),
